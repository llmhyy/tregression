...
public class CorrectTraceView extends TregressionTraceView {
		...
		Action action = new Action() {
			public void run() {
				...
				if (listViewer.getSelection() instanceof IStructuredSelection) {
					...
					ClassLocation correspondingLocation = diffMatcher.findCorrespondingLocation(node.getBreakPoint(),
							true);
					...
					if (otherControlDom != null) {
						...
						buggyTraceView.jumpToNode(buggyTraceView.getTrace(), otherControlDom.getOrder(),
								refreshProgramState);
					}
	...
	protected void markJavaEditor(TraceNode node) {
		...
		CompareTextEditorInput input = new CompareTextEditorInput(node, this.pairList, cfn.buggyFileName,
				cfn.fixFileName, getDiffMatcher());
	...
	protected void otherViewsBehavior(TraceNode correctNode) {
		if (this.refreshProgramState) {
			...
			try {
				stepPropertyView = (StepPropertyView) PlatformUI.getWorkbench().getActiveWorkbenchWindow()
						.getActivePage().showView(StepPropertyView.ID);
			} catch (PartInitException e) {
				e.printStackTrace();
			}
